...
namespace Orchestrator.Api.Infrastructure.Consumers;

/// <summary>
/// Consumes stock-related events for the Orchestrator service.
/// </summary>
public sealed class StockEventsConsumerHostedService : BackgroundService
{
    ...
    /// <summary>
    /// Starts consuming stock events from RabbitMQ.
    /// </summary>
    ...
    private async Task HandleMessageAsync(IModel channel, BasicDeliverEventArgs args, CancellationToken cancellationToken)
    {
        ...
        if (messageType == nameof(StockReservedEvent))
        {
            await ProcessAsync<StockReservedEvent, StockReservedEventHandler>(channel, args, body, cancellationToken);
            return;
        }

        if (messageType == nameof(StockReservationFailedEvent))
        {
            await ProcessAsync<StockReservationFailedEvent, StockReservationFailedEventHandler>(channel, args, body, cancellationToken);
            return;
        }
